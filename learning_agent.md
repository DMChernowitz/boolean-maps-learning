...
### When the limit exists: scaling priors, coin mixtures, and the ways to fail

This section is self-contained. The setup: hypotheses are the maps
$\phi : \{0,1\}^n \to \{0,1\}$ ($m = 1$ for clarity), $N = 2^{2^n}$
of them, with a prior $p_j$. A run asks the $2^n$ questions in
uniform random order and Bayes-updates on the true answers, the truth
itself drawn from $p$. Under this double average everything is
generated by one sequence, the mean block entropy

$$
G_\ell := \binom{2^n}{\ell}^{-1} \sum_{|S| = \ell} H(A_S),
$$

the joint entropy of $\ell$ answers, averaged over which $\ell$
questions were asked. Two exact laws (derived earlier in this
document): the expected surprisal collected after $\ell$ questions is
$G_\ell$, and the expected predictive entropy remaining in the answer
matrix is $(2^n - \ell)(G_{\ell+1} - G_\ell)$. The expected
cumulative leverage, destroyed over received, is then

$$
\langle L_\ell \rangle
= \frac{2^n G_1 - (2^n - \ell)(G_{\ell+1} - G_\ell)}{G_\ell}.
$$

The question of this section: measure time as the asked fraction
$x = \ell / 2^n$ and send $n \to \infty$. Does
$\langle L_{\lfloor x 2^n \rfloor} \rangle$ converge to a sensible
function $L(x)$, and what does the prior have to look like for it to?
(Note that "the prior" is really a family, one $p^{(n)}$ per size.)

**Step 1: the increments form a decreasing staircase.** Let
$\gamma_n(k) := G_{k+1} - G_k$. By the chain rule for entropy this is
the expected entropy of one more answer given $k$ random answers, and
"conditioning reduces entropy" (equivalently, Han's inequality for
subset entropies) makes it nonincreasing in $k$. It is also trapped
in $[0, m]$. Monotone and bounded: the best-behaved objects in
analysis.

**Step 2: where the integral comes from.** Telescoping,
$G_\ell = \sum_{k < \ell} \gamma_n(k)$, so

$$
\frac{G_{\lfloor x 2^n \rfloor}}{2^n}
= \frac{1}{2^n} \sum_{k < x 2^n} \gamma_n(k),
$$

which is exactly a Riemann sum of the staircase function
$t \mapsto \gamma_n(\lfloor t 2^n \rfloor)$ over $[0, x]$. *If* that
staircase converges pointwise to a limiting profile $\gamma(t)$
(necessarily nonincreasing), then, since everything is bounded by
$m$, the bounded convergence theorem lets the sum pass to the
integral:

$$
\frac{G_{\lfloor x 2^n \rfloor}}{2^n} \to g(x) := \int_0^x \gamma(t)\, dt.
$$

The laws become densities, received $\to g(x)$ and remaining
$\to (1 - x)\gamma(x)$ per question, and the leverage becomes

$$
L(x) = \frac{\gamma(0) - (1 - x)\,\gamma(x)}{g(x)}.
$$

Two endpoint checks by hand. As $x \to 0^+$, first-order Taylor
(or l'Hopital) with a differentiable profile gives numerator
$\gamma(0) - \gamma(x) + x\gamma(x) = x(\gamma(0) - \gamma'(0)) + o(x)$
against denominator $x\gamma(0) + o(x)$, so
$L(0^+) = 1 - \gamma'(0)/\gamma(0) \ge 1$: finite, and above one
precisely because the profile slopes down. At $x = 1$,
$L(1) = \gamma(0)/g(1) = H(M)_0 / H(p)$, the whole-run ratio.

**Step 3: the three assumptions, seen as failure modes.** The
derivation used exactly three things, and each one is a named way for
a prior to break it.

1. *Extensivity*, used at the division by $2^n$: the entropies must
   scale like the number of questions, $H(p^{(n)}) = \Theta(2^n)$,
   which means $\log p_j$ growing linearly in $2^n$ — a Gibbs shape,
   $p_j \propto e^{-\beta E_j}$ with extensive energy. Failure is not
   exotic; it is every prior whose learning lives on a sublinear
   clock. The freeze-out prior above has $H(p) = \Theta(\log n)$: its
   $\gamma(t)$ is $0$ for every $t > 0$, $g \equiv 0$, and $L(x)$ is
   formally infinite — the honest reading of "leverage diverging like
   $2^n / \log n$". The near-delta spike ($p \to 1$ after
   $n \to \infty$) fails the same way. The spike at *fixed* $p < 1$
   does not fail: its entropy is extensive, and it lands on the
   hyperbola law below.
2. *Self-averaging*, used at "the staircase converges pointwise":
   the expected entropy of a fresh answer given a random $t$-fraction
   must settle to a deterministic number. Symmetry guarantees it —
   for exchangeable priors $\gamma_n(k)$ is a single number by
   construction — and so does stationarity with decaying correlations
   (the Gibbs case). It can genuinely fail for families that change
   character with $n$ (say, alternating two structures on even and
   odd $n$): then the staircases oscillate and no single profile
   exists. Note what fails is uniqueness, not compactness — monotone
   bounded staircases always have convergent subsequences (Helly's
   selection theorem) — so such a family has several limiting
   $\gamma$'s rather than none.
3. *Positivity*, $\gamma > 0$ on $[0, 1)$: not needed for the limit
   to exist, but it decides whether $L(x)$ is regular. If the profile
   hits zero at some $x^* < 1$ — beyond a threshold the remaining
   answers are implied — then the destroyed density jumps by
   $(1 - x^*)\gamma(x^{*-})$ at that instant: an extensive avalanche
   of deduction compressed into $o(2^n)$ questions. The per-step
   leverage diverges at $x^*$, and the cumulative $L$ jumps
   discontinuously (it does not diverge unless extensivity also
   fails). The clean example is a rate-$\rho$ linear code prior:
   $\gamma = 1$ on $[0, \rho)$ and $0$ after, so $L(x) = 1$ before
   the threshold — no deduction at all, every answer a fair coin —
   and $L(x) = 1/\rho$ after it, in one step.

**Step 4: the coin mixture, from finite coins up.** The simplest
scaling prior, built by hand. Take $R$ coins with biases
$\theta_1, \ldots, \theta_R$ and mixing weights $w_1, \ldots, w_R$.
Nature picks coin $r$ with probability $w_r$, then answers all $2^n$
questions independently: $1$ with probability $\theta_r$. A truth
table $j$ containing $W_j$ ones (its Hamming weight) therefore gets

$$
p_j = \sum_{r=1}^{R} w_r\, \theta_r^{W_j} (1 - \theta_r)^{2^n - W_j}.
$$

Maps of equal weight get equal mass, so the prior is exchangeable
(invariant under permuting questions). Count the parameters: $R$
biases and $R$ weights summing to one, $2R - 1$ numbers, *independent
of $n$*. For contrast, the most general exchangeable prior assigns
one free mass to each Hamming class, $2^n + 1$ parameters, and a
general prior has $2^{2^n} - 1$. Three exponentially separated tiers
— and de Finetti's representation theorem says the first two merge in
the limit: any exchangeable family defined consistently for all $n$
*is* a mixture over coins, for some mixing measure. (Archetype A
above is the uniform mixing measure; this section is the
finite-atoms case.)

The profile is a step. The first answer sees only the mean coin:
$\gamma(0) = h(\bar\theta)$ with $\bar\theta = \sum_r w_r \theta_r$.
After $\ell$ answers the running frequency identifies the coin — the
law of large numbers, with the posterior on $\theta$ concentrating
exponentially fast for finitely many atoms (for smooth mixing
measures the approach is the $O(1/\ell)$ Clarke–Barron correction
quoted earlier) — so for every fixed $t > 0$,

$$
\gamma(t) = \bar g := \sum_r w_r\, h(\theta_r),
\qquad
g(x) = x \bar g,
\qquad
L(x) = 1 + \frac{c}{x},
\quad
c = \frac{h(\bar\theta) - \bar g}{\bar g}.
$$
...
its correlation store is extensive,
$C = H(M)_0 - H(p) \to 2^n \big(h(\bar\theta) - \bar g\big)$, the
same order as the Gibbs priors'. In *choreography* it is minimal.
Given $\theta$ everything is independent, so all connected structure
comes from uncertainty about one global scalar; the sufficient
statistic is the running count of ones; and the connected correlator
of any $k$ questions is the same number for every $k$-subset — the
$k$-th cumulant of the mixing measure. Every shell of the ledger is
populated, but each by a single value, and all of it is spendable
through the same one statistic. That is why the profile is a step:
the entire extensive store is cashed within the first $o(2^n)$
questions, and the hyperbola $1 + c/x$ is just the memory of that
initial burst diluting into the denominator. No mid-run peak (that
takes deep-shell knowledge cascading down, the Gibbs behavior), no
threshold avalanche (that takes algebraic implication, the code
behavior). So "one step above uniform" is exactly right about the
