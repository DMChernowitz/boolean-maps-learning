...
Ask the questions one at a time, in any order: by the symmetry of
the prior, which questions cannot matter, only how many. Write
$\mathcal S \subseteq \mathcal Q$ for the set asked so far, with
$|\mathcal S| = k$. A history remains on the spike branch exactly
when $\psi(\mathcal S) = 0^{mk}$, and exactly $N_k := N u^k$ maps
...
on the realized confirming branch, with $r_0 = p_0 = p$ the original
weight. For $0 \le k < Q$ the next zero answer, at any fresh
question $q \in \mathcal Q \setminus \mathcal S$, is predicted by
the special map and by $u N_k - 1$ of the surviving background maps,
giving
%
\begin{equation}
  \Pr\big(\psi(q) = 0^m \mid
  \psi(\mathcal S) = 0^{mk}\big)
  = r_k + (u N_k - 1)\,\omega_k
  = \frac{P_{k+1}}{P_k},
  \qquad
  \boxed{\;P_k\, r_k \;=\; p\;}
...
spike branch decreases while the conditional height $r_k$ increases,
and their product stays equal to the original mass. Equivalently
$\Pr(\psi = \phi_0 \mid \psi(\mathcal S))$ is a martingale under the
Bayes average over histories: it equals $r_k$ after $k$ confirmations
and zero after refutation, so its expectation is always $P_k r_k = p$.
...
\begin{tikzpicture}[x=1cm, y=1cm,
  ...
  \draw[flow] (1.75,0) -- (1.75,-1.2)
    node[alab, midway, left=1pt] {$\times\,0.475$};
  \draw[flow] (3.1,0) -- (7.6,-1.2)
    node[alab, pos=0.45] {$\times\,0.525$};
  \draw[flow] (8.75,0) -- (8.75,-1.2)
    node[alab, midway, right=1pt] {$\times\,1$};
  ...
  \draw[flow] (1.75,-3.1) -- (1.75,-4.3)
    node[alab, midway, left=1pt] {$\times\,0.724$};
  \draw[flow] (3.1,-3.1) -- (7.6,-4.3)
    node[alab, pos=0.45] {$\times\,0.276$};
...
the arrows carry their multipliers, and each row's map space is one
quarter of the row above. The outer columns track the branch
probabilities $P_k$ and $1 - P_k$, and the invariant $P_k r_k = p$ is
visible between the $P_k$ bars and the $r_k$ bars: the spike climbs
...
are $r_k/(1-r_k) = p(N-1)\big/\big[(1-p)(N u^k - 1)\big]$, and for
fixed $p > 0$,
%
\begin{equation}
  P_k = p + (1-p)u^k + O(N^{-1}),
...
correction disappears and $r_k/(1-r_k) \simeq u^{-k}p/(1-p)$: each
...
\end{equation}
%
for any question set $\mathcal S$ of size $k$. The entropy of that block
distribution is
%
\begin{equation}
...
%
\begin{equation}
  \big\langle H\big(M^{(k)}\big)\big\rangle = (Q-k)(G_{k+1}-G_k),
  \qquad
  \boxed{\;
  \langle L_k\rangle
...
\end{equation}
%
At any fixed $x > 0$ the conditional height $r_k$ on a surviving
...
$\langle H(M^{(\lfloor xQ\rfloor)})\rangle/Q \to (1-x)(1-p)m$ and
...
T))$ takes the same value $G_k$ for every $\mathcal S$ of size $k$.
...
$H(\psi(q)\mid\psi(\mathcal S)) \downarrow \mathbb E[h_2(\Theta)]$
and $H(\psi(\mathcal S))/k \to h_{\rm cond}$; the finite-component
